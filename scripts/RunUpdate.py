...
class UpdateManager(object):
    ...
    def _setup_pymysql_as_mysqldb(self):
        """
        Set up PyMySQL as a drop-in replacement for MySQLdb.
        This is needed on systems without python3-devel where mysqlclient cannot be compiled.
        Creates a sitecustomize.py file in site-packages to auto-install PyMySQL as MySQLdb.
        """
        try:
            # Check if mysqlclient is already installed and working
            import MySQLdb
            print("mysqlclient is already installed, skipping PyMySQL setup")
            return
        except ImportError:
            pass
        ...
        try:
            ...
            print("WARNING: PyMySQL is not installed, cannot set up MySQLdb replacement")
            return
        ...
        if not site_packages:
            print("WARNING: Could not find site-packages directory for PyMySQL setup")
            return

        sitecustomize_path = os.path.join(site_packages, 'sitecustomize.py')
        pymysql_shim = """# Auto-generated by OneDep installer
# Sets up PyMySQL as MySQLdb drop-in replacement
try:
    import pymysql
...
"""
        ...
        # Check if sitecustomize.py exists and already has our shim
        if os.path.exists(sitecustomize_path):
            with open(sitecustomize_path, 'r') as f:
                content = f.read()
            if 'pymysql.install_as_MySQLdb()' in content:
                print("PyMySQL already configured as MySQLdb replacement")
                return
            ...
            with open(sitecustomize_path, 'w') as f:
                f.write(pymysql_shim)

        print("Configured PyMySQL as MySQLdb drop-in replacement in {}".format(sitecustomize_path))
    ...
    def updatepyenv(self, dev_build):
        ...
        # Only configure private index if CS_* variables are properly set
        if cs_url and cs_user and cs_pass:
            ...
            print("WARNING: CS_USER, CS_PW, or CS_URL not configured - skipping private package index setup")
            print("WARNING: Internal wwPDB packages will not be available from private index")
            self.__exec('pip config --site set global.extra-index-url "https://pypi.anaconda.org/OpenEye/simple"')
        ...
        # Set up PyMySQL as MySQLdb replacement if mysqlclient cannot be built
        # (e.g., when python3-devel is not available on RHEL9)
        self._setup_pymysql_as_mysqldb()
